...
class SimpleButton:
    ...
    STATE_IDLE = 'idle'
    STATE_ARMED = 'armed' # button is up, mouse not over button
    ...
    def __init__(self, window, loc, up, down, callback=None):
        ...
        self.callback = callback # function to call when button is clicked
        
    
    def handleEvent(self, eventObj):
        # This method will return True if user clicks the button.
        # Normally returs False
        
        if eventObj.type not in (MOUSEMOTION, MOUSEBUTTONUP, MOUSEBUTTONDOWN):
            # The button only cares about mouse-related
            return False
