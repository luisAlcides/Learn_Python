...
class Balloon(ABC):
    ...
    @abstractmethod
    def __init__(self, window, maxWidth, maxHeight, ID, image, size, nPoints, speedY):
        self.window = window
        self.ID = ID
        self.balloonImage = image
        self.size = size
        self.nPoints = nPoints
        self.speedY = speedY
        if not Balloon.popSoundLoaded:  # load first time
    ...
    def update(self):
        self.y = self.y - self.speedY  # update y position
        self.balloonImage.setLoc((self.x, self.y))
    ...
    def __del__(self):
        print(self.size, 'Ballon', self.ID, 'is going away')
...
class BalloonSmall(Balloon):
    ...
    def __init__(self, window, maxWidth, maxHeight, ID):
        image = pygwidgets.Image(window, (0, 0), BalloonSmall.balloonImage)
        super().__init__(window, maxWidth, maxHeight, ID, image, 'Small', 30, 3.1)
...
class BalloonMedium(Balloon):
    ...
    def __init__(self, window, maxWidth, maxHeight, ID):
        image = pygwidgets.Image(window, (0, 0), BalloonMedium.balloonImage)
        super().__init__(window, maxWidth, maxHeight, ID, image, 'Medium', 20, 2.2)
